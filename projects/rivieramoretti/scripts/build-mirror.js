...
function buildViewerConfigScript() {
  const internalRouteMap = Object.fromEntries([
    [".", "./"],
    ...PAGE_DEFS.filter((page) => page.slug).map((page) => [page.slug, `${page.slug}/`]),
  ]);

  return `<script>(function(){const propsId="__RM_PROPS__";const basePath=(function(){const pathname=new URL(document.baseURI).pathname||"/";return pathname==="/"?"/":pathname.replace(/\\/+$/,"")+"/";})();const propsNode=document.getElementById(propsId);if(propsNode&&propsNode.dataset&&propsNode.dataset.content){try{const props=JSON.parse(propsNode.dataset.content);props.isDownloadedSource=true;props.homepageRewrite=false;props.exportBasePath=basePath;props.config=props.config||{};props.config.root=basePath;propsNode.dataset.content=JSON.stringify(props);}catch{}}if(window.ServerData&&typeof window.ServerData==="object"){window.ServerData.isDownloadedSource=true;window.ServerData.homepageRewrite=false;window.ServerData.exportBasePath=basePath;window.ServerData.config=window.ServerData.config||{};window.ServerData.config.root=basePath;}window.viewerConfig=window.viewerConfig||{};viewerConfig.readymagTracker=false;viewerConfig.isProjectOwnedByRM=false;viewerConfig.userTracker=false;viewerConfig.isDomainViewer=true;viewerConfig.customDomainProfile=window.ServerData&&window.ServerData.mags?window.ServerData.mags.domainForUser:false;viewerConfig.homepageRewrite=false;viewerConfig.isDownloadedSource=true;viewerConfig.exportBasePath=basePath;window.RM=window.RM||{};window.RM.config=window.RM.config||{};window.RM.config.root=basePath;const internalRouteMap=${JSON.stringify(internalRouteMap)};const projectId=${JSON.stringify(PROJECT_ID)};const hasOwn=function(key){return Object.prototype.hasOwnProperty.call(internalRouteMap,key);};const routeFromRaw=function(raw){if(typeof raw!=="string"){return null;}const value=raw.trim();if(!value||value.startsWith("#")||value.startsWith("mailto:")||value.startsWith("tel:")||value.startsWith("javascript:")){return null;}if(value==="."||value==="./"||value==="/"){return "./";}const compact=value.replace(/^\\.?\\//,"").replace(/\\/+$/,"");if(compact&&hasOwn(compact)){return internalRouteMap[compact];}try{const url=new URL(value,document.baseURI);const host=(url.hostname||"").toLowerCase();if(host&&hasOwn(host)){return internalRouteMap[host];}const pathname=(url.pathname||"/").replace(/^\\/+|\\/+$/g,"");if(!pathname){return "./";}const segments=pathname.split("/").filter(Boolean);if(!segments.length){return "./";}if(segments.length===1&&hasOwn(segments[0])){return internalRouteMap[segments[0]];}if(segments[0]===projectId&&segments.length===1){return "./";}if(segments[0]===projectId&&hasOwn(segments[1])){return internalRouteMap[segments[1]];}}catch{}return null;};const hrefForRoute=function(route){return new URL(route,document.baseURI).href;};const normalizeAnchor=function(anchor){if(!anchor||typeof anchor.getAttribute!=="function"){return;}const rawHref=anchor.getAttribute("href")||anchor.href||"";const route=routeFromRaw(rawHref);if(!route){return;}const nextHref=hrefForRoute(route);if(anchor.getAttribute("href")!==nextHref){anchor.setAttribute("href",nextHref);}if(anchor.target&&anchor.target.toLowerCase()!=="_self"){anchor.setAttribute("target","_self");}};const normalizeNode=function(node){if(!node){return;}if(node.nodeType===1&&node.matches&&node.matches("a[href]")){normalizeAnchor(node);}if(node.querySelectorAll){node.querySelectorAll("a[href]").forEach(normalizeAnchor);}};const handleClick=function(event){const anchor=event.target&&event.target.closest?event.target.closest("a[href]"):null;if(!anchor){return;}const route=routeFromRaw(anchor.getAttribute("href")||anchor.href||"");if(!route){return;}const nextHref=hrefForRoute(route);if(anchor.href!==nextHref){anchor.setAttribute("href",nextHref);}if(window.location.href===nextHref){event.preventDefault();return;}event.preventDefault();window.location.assign(nextHref);};const startInternalLinkGuard=function(){normalizeNode(document);const observer=new MutationObserver(function(mutations){for(const mutation of mutations){if(mutation.type==="attributes"){normalizeAnchor(mutation.target);continue;}for(const node of mutation.addedNodes){normalizeNode(node);}}});observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:["href"]});document.addEventListener("click",handleClick,true);window.addEventListener("load",function(){normalizeNode(document);},{once:true});};if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",startInternalLinkGuard,{once:true});}else{startInternalLinkGuard();}})();</script>`;
}
